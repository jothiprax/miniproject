...
class OnlineBanking{
    ...
    {
        try{
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306","root","Manoj@2004");
    ...
    public void registerUser(String userName,String password)
    {
       try {
        st = con.createStatement();
        rs = st.executeQuery("Select userName from Account where userName= '"+userName+"'");
        if(!rs.next())
        {
            PreparedStatement register = con.prepareStatement("Insert into Account values(?,?,?,?,?)");
    ...
    public int logIn(String userName,String password) throws InvalidUserNameException,InvalidPasswordException
    {
        try{
            ...
            if(rs.next())
            {
                ResultSet pwd = st.executeQuery("Select userName from Account where password= '"+password+"'");
                if(!pwd.next())
                {
    ...
    public void withdraw(String userName,double amount) throws InsufficientBalanceException,InvalidPasswordException
    {
        try{
            if(con!=null){
            PreparedStatement wd = con.prepareStatement("Update Account set balance = balance - ? where userName = ? and balance >= ?");
            ...
            st= con.createStatement();
            rs=st.executeQuery("select password from Account where userName='"+userName+"'");
            rs.next();
    ...
    {
        try{
            PreparedStatement dp = con.prepareStatement("Update Account set balance = balance + ? where userName = ?");
    ...
    public void checkBalance(String userName) throws InvalidPasswordException
    {
        try
        {
            PreparedStatement bal=con.prepareStatement("Select balance from Account where username=?");
    ...
    {
        try{
            PreparedStatement detail=con.prepareStatement("Select * from Account where userName= ?");

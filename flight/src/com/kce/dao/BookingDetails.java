...
import com.kce.util.FMutil;

public class BookingDetails {
	Static Scanner sc = new Scanner(System.in);

	public static void Customer(BookingDetails fm) {
		try {

			Connection con = DButil.getConnection();
			PreparedStatement stmt = con.prepareStatement("INSERT INTO cus_details (name, phNo, flightNo, noOfSeats) VALUES (?, ?, ?, ?)");
			stmt.setString(1, bd.getName());
			stmt.setInt(2, bd.getPhNo());
			stmt.setInt(3, bd.getflightNo());
	...
	public static void displayTable() {
		try {
			...
			System.out.println("--------------------------------------------------------------------------------------");
			System.out.printf("| %-20s | %-20s | %-20s | %-20s |%n", "Name", "Phone Number", "flight No","No of Seats");
